...
export const registrationUserByAdmin = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const html = await ejs.renderFile(
        path.join(__dirname, "../mails/activation-by-admin-mail.ejs"),
        data
      );
      ...
      try {
        await sendMail({
          ...
          template: "activation-by-admin-mail.ejs",
          data,
        });

        res.status(201).json({
          success: true,
          message: `Account created successfully. Login details sent to ${newUser.email}!`,
        });
      } catch (error: any) {
